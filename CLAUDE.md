...
- **Closed-item post-mortems:** `completed/<issue-number>.md` per closed issue (e.g. `completed/13.md`). Read the matching entry before re-deriving decisions on territory that overlaps a closed item: `grep -r 'pattern' completed/`. Legacy pre-migration entries (the original "Item N" 1–39 from before the GitHub-issue convention) still live in `COMPLETED.md` at repo root and are searchable side-by-side until a future migration ships.
