...
- **Commit progress within #17:**
  ...
  - C5 next: `main.py` stats integration — load `sender_stats.json`, derive `newsletter_senders` set from `newsletter_stats.newsletter_senders(stats)`, pass to `extract_events`, update stats after extraction from the candidates list + `new_emails` (so zero-event messages contribute a 0 to the rolling window), compute outlier alerts via `newsletter_stats.outlier_alerts(...)`, thread alerts into `scripts/process_events.py` via a new `--outlier-alerts <path>` tempfile, save stats atomically. Dry-run reads but does not write the stats file (mirrors `prior_events.json` posture per #13).
