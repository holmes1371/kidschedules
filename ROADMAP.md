...
**2026-04-16 (session 3 — #11 design note landed + soft-delete infra)**

- **In progress: #11** — card redesign. Design note landed at `design/card-redesign.md` (`70c172a`), built on the session-2 mockup at `design/card-redesign-mockup.html` (`787abd0`). All Layout A decisions are locked inside the design note — do not re-litigate from ROADMAP body bullets. ROADMAP #11 stays `[ ]`; flip to `[~]` when the implementation commit lands.
- **Next step (pick up here):** implement Layout A in `scripts/process_events.py` per the design note. Specifically: two new pure helpers `_is_all_day` and `_is_suppressible_location`; template rewrite in `_event_card` + parallel changes in `_undated_card`; CSS additions for `--everly` / `--isla` variables, `.meta-strip`, `.child-chip` (everly/isla), `.time.allday`, `.event-location`, `.event-audience`; removal of orphaned `.badge` / `.event-meta` / `.child` / `.source` / `.event-date` rules; ingest-side flip of `"Time TBD"` / `"Location TBD"` defaults in `build_events` to empty strings. Extend `tests/test_process_events.py` with ~12 new cases *in the same commit* (red tests would block merge); add `fixtures/test/card_redesign.json` mirroring the mockup's six variants. Hard constraints from the design note: do NOT touch `agent.py` (all-day canonicalization is render-side), do NOT touch `render_event` / `render_undated` / `render_body` (plain-text email stays verbatim), do NOT migrate the category rail from inline `style="border-left: ..."` to CSS classes, do NOT address the stale-fixture flag in this commit.
- **Infrastructure landed mid-session: `.to_delete/` soft-delete convention** (`17cdd87`). The FUSE mount refuses `unlink` but permits `rename`, so `rm` fails with `Operation not permitted` even under sandbox-disable; every interrupted git op leaves `.git/index.lock` or `.git/HEAD.lock` behind and blocks the next command. Fix: `mv` into `.to_delete/` at repo root (kept in tree via `.gitkeep`, contents `.gitignore`d). Session-discipline block below points at `design/soft-delete-convention.md` which covers the rule, stale-lock recovery, and the `.git/index` corruption recovery ritual (which this session hit once and walked through live). Tom empties `.to_delete/` from Windows File Explorer periodically. Ten files from this session's recovery are currently parked there — fine to leave until Tom sweeps.
- **Stale fixture flag (carry-over)**: `fixtures/sample_candidates.json` contains "Glasgow Middle School / Isla" which does not match reality. Still not a blocker for #11; remains a one-off cleanup pass after the redesign lands so `scripts/dev_render.py` produces realistic previews.
- **Open thread — Cowork permission re-prompts**: every `mv` / compound git command this session re-prompted Tom despite "always allow" clicks, because command-string matching treats `$(date +%s)` timestamps and HEREDOC commit messages as unique each time. Fix proposed but not implemented: broader allowlist entries like `Bash(git:*)` and `Bash(mv:*)` in `.claude/settings.local.json`. Tom may paste these in himself; if he asks for help, that's the shape.
- **Earlier session closures (for context)**: session 2 shipped the Layout A mockup (`787abd0`) and the #12/#13 ROADMAP swap including the teacher-roster subtask. Prior sessions closed #16 Node 24 action upgrades (`ea081da`), #9 footer tempo copy (`756428c`) + archive-infrastructure rip (`2640c4b`), #10 Monday-only draft gating (`65c86f3`).
- **Repo state**: 224 tests passing (no test changes this session — pure design + infra). Three commits this session: `70c172a` (design #11), `17cdd87` (soft-delete infra), and one more from this close-out. Tom pushed all three live; worktree clean at session end.
