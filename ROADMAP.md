...
**2026-04-17 (session 7 — #18 undated-card ignore affordance pending visual QA)**

- **Just shipped: #18 Ignore affordance for undated "Needs Verification" cards.** Two commits on `main`: `41505aa` (fixture + `_undated_card` markup + 4 render tests + id-collision test + ROADMAP flip `[ ]` → `[~]`), `aade8aa` (extend server-side `ignored_n` count to span display + undated + 2 toggle-count tests). Status left at `[~]` pending Tom's live QA — the markup is pinned by tests but browser behavior on ignore/unignore click paths wasn't exercised. **Push and visual-QA checklist:** (1) open Needs Verification section, click Ignore on a card → card fades out, Show-ignored toggle appears with count 1, click toggle → reveals the hidden card with an Unignore button; (2) click Unignore → card returns, counter decrements to 0, toggle disappears; (3) reload page → local storage persists the ignore, card reloads hidden with toggle visible; (4) verify the Apps Script "Ignored Events" sheet gets a new row with `date=""` — empty is expected and tolerated by `_handleIgnore` (no date validation).
...
- **Design decisions locked in for #18** — no design note written (ROADMAP explicitly waived it; "straight port of the existing pattern"). Key choices worth knowing:
  - **Id-collision contract pinned.** `_event_id(name, "", child)` and `_event_id(name, "YYYY-MM-DD", child)` are disjoint by construction (sha1 input differs on the middle segment). Test: `test_event_id_undated_vs_dated_no_collision` in `tests/test_process_events.py`. Needed because undated ignores now share a storage/audit pipeline with dated ignores; the hash-input distinction is load-bearing.
  - **Ignore-sender stays out of scope on undated cards.** Undated cards that reach "Needs Verification" are typically the ones the extractor couldn't ground fully, so their `sender_domain` (when present) is unreliable. The new `_undated_card` deliberately does NOT emit `data-sender` even when the source event has one — tested by `test_render_html_undated_card_has_no_ignore_sender_button` against a fixture entry with `sender_domain="laes.org"`.
  - **JS handler unchanged.** The click router and hydration IIFE already selected on `.event-card[data-event-id]` generically — once undated cards carry the attribute they slot in automatically. `setIgnored`/`setActive` don't touch the yellow left-rail color, only `display` + `ignored` class + button swap, so the undated card's visual identity survives toggling.
  - **`ignored_n` now sums display + undated.** Deliberate deviation from "straight port" — without it, an undated card ignored on a prior run would reload hidden on the next build with no toggle present to reveal it. Tom signed off this deviation in the plan.
  - **Apps Script audit row for empty date.** `_handleIgnore` writes `date || ''` verbatim with no validation (audited in this session). The sheet gets a row with an empty Date column; `_listIgnoredEvents` echoes that back; `_load_ignored_ids` only extracts `id`. End-to-end tolerant. No Apps Script change needed.
- **Shape of the `_undated_card` change.** `scripts/process_events.py::_undated_card` now mirrors `_event_card`'s top-action-row pattern: `data-event-id` on the outer div, `data-ignored="1" data-ignored-reason="event"` when `is_ignored`, and an `event-actions-top` div wrapping the Ignore/Unignore button. The `.ignore-status` placeholder div was intentionally NOT added — it's `:empty` → `display:none` on dated cards anyway, and omitting it keeps the undated markup minimal. Button attributes `data-event-name` + `data-event-date` use the normalized values from `classify()` so `data-event-date=""` is emitted on undated cards; the JS handler's `getAttribute("data-event-date") || ""` normalizes either empty-or-missing to `""` for the POST.
...
**Implementation status (2026-04-17, pending Tom's live QA):** `_undated_card` markup mirrors `_event_card`'s top-action-row — `data-event-id` on the outer div and an `event-actions-top` div with the Ignore/Unignore button. JS handlers already select on `.event-card[data-event-id]` so no router/hydration changes were needed. Server-side `ignored_n` was extended to span display + undated so an undated card ignored on a prior run can be revealed on reload. Apps Script `_handleIgnore` tolerates empty `date` without change. Id-collision contract pinned by `test_event_id_undated_vs_dated_no_collision`. See the 2026-04-17 session-7 summary above for the QA checklist.
