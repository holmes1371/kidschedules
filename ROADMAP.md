...
- **Repo state at session end:** 250 tests passing (was 238 at session start; +3 roster tests + 8 chip tests + 1 activities-clause test = 12). **Nine commits local on `main`, ahead of `origin/main` and not yet pushed.** Chronological: `f0976f6` (design note), `fd0c264` (roster subtask), `399d383` (chip row), `0f1c3f5` (ROADMAP flip → [~]), `4cbc8dc` (drop .gitkeep), `c27ee03` (flip-on-approval discipline), `e0547c6` (Glasgow retirement), `cd816f0` (activities key), plus this ROADMAP close-out commit. Worktree clean aside from gitignored `docs/dev_preview.html`. `.to_delete/` accumulated more stale `.git/*.lock` files from this session's git churn — safe to leave until Tom sweeps.
