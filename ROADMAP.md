...
- **Commit progress within #17:**
  ...
  - C6 next: `scripts/process_events.py` grows `--outlier-alerts <path>` reading a JSON list of alert dicts; the weekly digest (text + HTML) gains a "⚠️ Possible under-extraction" section above the event list, one line per alert with the message ID verbatim. `main.py` writes a tempfile of `alerts` and passes `--outlier-alerts` to `run_script("process_events.py", ...)`. Digest-block rendering tests in `tests/test_process_events.py`. Mon-only surfacing via the existing digest cadence (Wed/Sat runs suppress the draft per #10, so alerts there remain stdout-only).
