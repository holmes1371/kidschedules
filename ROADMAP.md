...
- **Commit progress within #17:**
  ...
  - C7 next: GitHub Actions workflow YAML needs to restore/save `sender_stats.json` from/to the `state` branch alongside `events_state.json`, `prior_events.json`, and `processed_messages` counters. `.gitignore` gets `sender_stats.json` so local development doesn't churn the worktree. Workflow-file edit is the only code change — it's a surgical addition to the existing restore/save steps. Anchor commit: `ac4ae3b` (#13 C2) shows the pattern that added `prior_events.json` to the same restore/save plumbing. Karpathy rubric tier: workflow/cron = **ultrathink** per the global discipline.
